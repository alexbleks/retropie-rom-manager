...
function resizeToVariable($sourceImage,$newHeight,$newWidth,$destImage)
{
    ...
    ob_start();
    imagejpeg($tmp_img,$destImage,100);
    $i = ob_get_clean(); 
    // Save file
  ...
  /*  echo "Upload: " . $_FILES["file"]["name"] . "<br>";
    ...
    if (file_exists("SNES/roms/" . $_FILES["file"]["name"]))
      ...
      "SNES/roms/" . $_FILES["file"]["name"]);
		...
		// resize Image
		$ImageFileName = "SNES/roms/".$_FILES["file"]["name"];
...
if ($counter == 0)
{
	...
	$exec = 'INSERT INTO games (game_title,md5,console) VALUES ("No Title Yet","'. $md5hash .'", "Super Nintendo (SNES)")';					
	$go = $db->exec($exec);
...
$query2 = "SELECT * FROM games WHERE md5='".$md5hash."'";
...
<form action="snes_view.php?hash=<?php echo $md5hash ?>" method="post" enctype="multipart/form-data">
<title>RetroBox ROM Manager</title>
<h1><center><font color=white>Super Nintendo</font></center></h1><br>
...
<?php
	...
	$ImgPathFile = "/SNES/roms/".$ImageFile;
